...
# 13. 保留だった検討事項（★2026-09-16 に一部着手済み）

**現況**：13.1 の手編集検知ガードは **実装・push 済**（`9d66b58d3`）。13.2 の「正本を HTML へ
これ以上寄せるか」は **しないで決着**（再提案しない）。13.2b（`renumber_eyebrow` /
`delink_missing` の撤去）は **次に importer を触るとき一緒に**。残る未決は 13.3。
**未決のものを勝手に始めない**のは従来どおり。
