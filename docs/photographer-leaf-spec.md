...
**素材に生没年が無くても、空欄・年代文字列のまま通さない — Daisuke決定 2026-08-31**
渡された素材（Daisuke 提供・ChatGPT 生成のどちらでも）に生没年が入っていない場合、
**作業側（Claude）が調べて入れる**。「素材に無いから空のまま」は不可。
探す順序・表示3ヶ所・デュオ／団体の表記規則・JSON-LD の揃え方は
`docs/importer-scaffold-inject-spec.md` §14-C「`Years` の『既存維持』は中身がある場合だけ」
に集約してある（新規ページ作成でも update でも同じ規則）。
一次情報で確定できないときだけ据え置き可。その理由を run-log に書く。**推測で年を作らない。**
