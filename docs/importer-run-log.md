...
- **範囲 / 実測**：既存128枚を `preflight.py` の共通抽出規則で再列挙し、監督指定49枚を面ごとに判定。手編集17枚＋国別生成25枚の計42枚を変更。監督差分監査後、年代固有の主題語を温存する4枚を再是正したため、非前方一致の最終値は128→91。年代面で card-data のタグ数が表示数Nに満たない7枚と、国別Mali 1枚も情報消失回避のため温存。
- **手作業点**：① `cards-archive.html` 11枚・年代3面6枚のタグ本文だけを置換、②国別12面をスコープ付き生成し、変更25枚が対象23＋監督承認済み2枚だけであることをプレビュー／実生成後に照合。カード枚数・slug順はJA/EN全対象面で不変。
- **面 / フィデリティ**：HTML 30面（JA=cards-archive 1＋年代3＋国別12、EN=国別12＋年代2）＋本ログ。本文・リンク・href・出典・SEO・カード増減・並び順の変更0。フィデリティ N/A。
- **bug / engine / commit**：サイトengineの変更0。適用時に短い同型文脈のパッチが別カードへ当たったが、生成前監査で検知しHEADから期待状態を再構成して解消。commit未作成。wall-time：（Daisuke記入）
- **監督差分監査後の是正**：年代4枚を追加修正。`tokuko-ushioda` は「私写真・タイポロジー写真」、`annie-leibovitz` は「ステージド写真・ポートレート」、`goldin` はHEADの「私写真・エイズ危機」へ復元、`becher` は「タイポロジー写真・工業建築」とし、年代固有の主題語を前方一致より優先。EN年代1970/1980をスコープ再生成した。
- **③ card-data 漏れ補完**：`salgado.tags` の末尾へ `社会ドキュメンタリー` を追加。先頭4語と全HTMLは不変で、非前方一致91枚も不変。併せて同型候補6件（stieglitz=`モダニズム` / steichen=`ストレート写真` / moholy=`モダニズム` / andre-kertesz=`決定的瞬間` / toyoko-tokiwa=`ドキュメンタリー` / wall=`大判カラー写真`）もDaisuke裁定により各tags末尾へ補完。6名のJAカード45出現は変更前後の表示タグ列SHA-256が一致し、既存の前方一致面の破損0・非前方一致91枚不変。HTML・生成器の変更0。
...
- **運用決定（2026-09-11・Daisuke）**：**importer 側は直さず、素材側で担保する。**
  ChatGPT へ表記ルールのプロンプトを常時渡し（spec §13-0）、受け取った Claude が
  着手前にマーカーを検分する（spec §14 A-0）。揺れていたら差し戻す。
  生成側の数量突き合わせガードと正規表現の緩和は**採らない**（再提案しない）。
...
- **commit**：あり（監督監査後 29f6c1594）。
- **wall-time**：16分（Daisuke実測。1名あたり4分）

## 2026-07-22 — 10名バッチupdate（ChatGPT新素材で本文全刷新・Opus監督/Codex実装）

- **種別**：update×10。カード・年代・国・運動・スターマップ・archive面は不触。分業＝Opus（本会話）が監督（ファイル↔slug対応確定・パイロット監査・§REL差し替え判断・ガード確認）、Codex MCPが機械実装（importer5ステップの反復実行・報告）。トークン節約のため fable 不使用時の代替構成。
- **対象と素材**：素材 `photography history/re-photographer/0722/`。10名とも JA/EN 対称（節8・cite数一致・レビューspan 0・裸text §REL多め）の clean v5.1。**torbjorn の正規slugは既存 `torbjrn-rdland`（øが欠落した既存slug）＝新規作成せず据え置き**。
- **パイプライン（1名あたり）**：①`--update-existing --slug S --ja J --en E --prepare`（backup+spec）→②同 `--apply --force`（carry-forward を JA HTML へ・内部で check_content_loss 通過）→③`--merge-to-en E --slug S --apply`（en-content.json へ skip-empty マージ・他slug byte不変assert・works ui-terms は add分のみ自動書込）→④`build_photographers_en.py --slug S`。パイロット=bruno を本会話で通し検証後、残り9名を Codex へ同一レシピで委譲。
- **手作業点（§REL 意図差し替え5名 + 辞書1行）**：naoya/takashi-homma/torbjrn-rdland/wang-qingsong/yang-fudong は④が「Related link 削除」保護で SKIP。実測すると **新素材§RELのリンク人物＝現JSON site_directory と完全一致**（例 wang=Yang Fudong/Morimura/Sherman）、SKIP対象は旧EN HTMLに残る**単一の旧movementリンク**（Conceptual Art / Typological Photography / Color Photography / Staged Photography）で、いずれも新素材§RELに無く JA §REL も同じく差し替え済み＝日英対称の意図的置換と確認。5名を builder `--force`。builder が SKIP せず一発ビルドしたのは bruno/paul-graham/philip-lorca-dicorcia/simon-norfolk/sophie-calle の5名。
- **bug / engine改良**：既存穴1件＝`build_photographers_en.COUNTRY_JA_TO_EN` にノルウェー欠落（旧EN backup も `>ノルウェー<` のまま＝再生成起因ではない既存潜在バグ・torbjorn の hero Country のみ実国名）。`'ノルウェー': 'Norway'` を add-only 補完し untranslated 警告解消（他ページ影響なし・add-only）。works ui-terms は wang-qingsong 2件 add-only（森美術館-Follow Me / 作家公式-作品アーカイブ）。
- **面（tracked 23）**：JA10＋EN10＋en-content.json（変更エントリは対象10slugのみ・追加/削除0を python 差分で確認）＋ui-terms.json（+2）＋build_photographers_en.py（Norway辞書+1行）。intentional-replacements.json 不要（§REL差し替えは check_content_loss 非対象・本文出典の消失なし）。カード・分類・共通CSS・filter/sort UI差分なし。
- **フィデリティ**：合計本文 8495→62751字、unique出典 35→213。全員 dangling 0・フィデリティ非減少。個別本文=bruno 1020→5048 / naoya 958→6564 / paul-graham 1019→7217 / philip 947→7427 / simon 1017→5377 / sophie 749→7272 / takashi 751→7062 / torbjorn 692→5507 / wang 673→5988 / yang 669→5289。EN photobooks_html / site_directory_html / footer_html / jsonld は skip-empty carry-forward。
- **検証**：check_content_loss OK（本文消失・書き換えなし）。preflight EXIT 0（FAIL/✗ 0）。全10 slug builder `--dry-run` SKIPPEDなし。preflight WARN はJA/EN各10ページの data-nosnippet 1減（9→8・8→7）のみ＝プレースホルダ/revisionマーカー除去の正当減少。GA/canonical/hreflang/OG/JSON-LD 維持。EN §REL は5名とも新素材の人物リンクへ差し替え確認。素材原本20ファイルのSHA-256は import 後も不変。対象外巻き込み・巻き戻りなし。
- **素材SHA-256**：bruno JA `dc51bb4af01acaa3687c6b02af25fa44aab40298389fa7be42b4cbb414fcef67` / EN `4999108bc2fbbb0ecba3206481fcfe68575e56f0efe81e1ab5c435386e077693`、naoya JA `ae182d598f02df1fa96b7b39c2c9dc94d8240a60b3b117d6db81a38aa6e44900` / EN `5b043239772dc79e3bd7196db0ec30625794b67f310884d0cf03236c3977b832`、paul-graham JA `7363e75b9d967f6b81a033ce858917e84d6c7e1fdf0c256b7061100e1813ce18` / EN `3dc5870b3f210fc7c12409da2a7b70f9da968862384e38538f357ae036041b99`、philip-lorca-dicorcia JA `804df09fb717e0533b08141de4b7b522bfb81c65297c8c9e010faba0406a961d` / EN `a80d728f6a95a40494bc1eae2340bcf1ffbf78daf5dd9d2ed3c0b0ecfec4a254`、simon-norfolk JA `8cfdeba766a250fee28663f9c4a60c142697ffd2bac0c3f50304b729d8827f66` / EN `0299ba82fdbc92a5ac140d69843c7a3929d48a43c1859fed197561fcf14a9afa`、sophie-calle JA `7a5db8a1e12e9acf3c2ebfc49388e858973e127d7f18c8c66d507f8fad0bf2bb` / EN `3277c8fabd64441eb8a5758c8f03a2163d42159bfa4d04085bf1865aaef27aef`、takashi-homma JA `bdc2a7d7b12f6d8acc9da2c870e39e4a8dce80c1ffc1fa9d07a43decf69afc92` / EN `b6a164e596a2c8577ec4acbf02121dbb8714e9fc719ee8ecab414240f67ee7e4`、torbjorn JA `cd71ba247c7f984440705c6513428720d299dc772bfb5f7d4523dd5d24e19662` / EN `b9cd21431b9c7c9386673e7b7a53919409239f5e60b53b755992e2c5ee5f3049`、wang-qingsong JA `7758ae377fc33c237204e11ccdc054137371db94aaf9b467f37483f59bb2b7fa` / EN `4e2d9c5c2d7235940b7f5d3a541a2a14921fa39a228f9167774a40d50ae71f7a`、yang-fudong JA `d5eaa0d7d59f3de5df2b281958b3bd53332e88c7ce5944b8d59a012891f398fe` / EN `6d9b551263b71668e86e468b41cdd3e28d7b1f837fa3ca00f0c4dfe06b50ce14`。
...
## 2026-08-20 — 0820写真家7名 update バッチ完了

- **種別 / 範囲**：update。`arno-nollen` / `multiplicity` / `nate-lowman` / `ohio` / `oliver-musovik` / `paul-albert-leitner` / `wangechi-mutu` のJA正本HTML、EN正本JSON、生成EN HTMLを0820素材（14ファイル）で刷新。`multiplicity`（イタリアのコレクティブ）/ `ohio`（ドイツのユニット）/ `wangechi-mutu` は本文ほぼ空のスタブ（JA 4字 / EN 12字）で、0813の `collectif-fact` / `g-r-a-m` と同型。
- **bug / engine改良**：engine変更0。① 素材に0813と同性質の `rev` クラス 1,582トークン（構造要素への追加クラス。`paul-albert-leitner` のJA/ENのみ0）。unwrapせず作業コピーのclass属性から `rev` トークンだけを除去して要素を保持。② **新規事象＝`prep-block` の orphan class FAIL**。EN §RELのリンク可能項目が0の3slug（`multiplicity` / `ohio` / `wangechi-mutu`）で builder が標準の `<div class="prep-block" data-nosnippet>In preparation</div>` を生成する一方、新スキャフォールドのJA側は `prep-block` を使わないため `.prep-block{...}` の定義が落ち、`preflight.py` の `check_orphan_class_tokens()` がHARD FAILした。**サイト標準を実測して解消**：`en/photographers/` で `class="prep-block"` を持つ非backupページ24件のうち21件は同一ページ内にCSSも定義しており、「markupを持つページはCSSも持つ」が標準。JA正本inline `<style>` の `@media(max-width:640px){.ph-sources{...}}` 直後・`/* SIDEBAR SEARCH */` 直前に既存とbyte一致の201 bytesブロックを保持し、EN再生成でENへ伝播させた。`ALLOW_ORPHAN_CLASS` は不使用（全ページ横断でガードが緩むため）。`styles/` 共通CSSへの追加も禁止面につき不採用。JAはdef=1 / use=0、ENはdef=1 / use=1。
- **手作業点**：§RELを5リンク手是正（Arno=`Rineke Dijkstra`、Nate=`Roe Ethridge` / `ピクチャーズ世代`、Oliver=`J. H. Engström`、Paul=`ニューカラー`）。人名slugはすべて `card-data.json` の `nameJa` / `nameEn` から解決。JA裸項目32件はcard-dataとlocale別movement実ファイルを照合し、リンク可能項目0を実測。
- **carry-forward / Years確定**：hero眉 / Country / Movement / Periodは全7名で既存維持。YearsはDaisuke指示により今回のみ§14 Cを上書きし、空欄だった4名を一次情報で確定済みの値へ更新（`arno-nollen`=`1964–2022`、`nate-lowman`=`1979–`、`oliver-musovik`=`1971–`、`paul-albert-leitner`=`1957–`）。JA dl / JAサイドバー / EN dl / ENサイドバーの16箇所を同期し、JSON-LDもArno=`birthDate 1964` / `deathDate 2022`、他3名=`birthDate`のみで整合。`multiplicity`=`1990s–` / `ohio`=`1995–` / `wangechi-mutu`=`1972–`は据え置き。Amazonリンク・検索URLはJA/EN合計0→0、新たに増えた検索URLは**0本**、ASIN解決なし。
- **Related削除ガード**：承認済み`--force`は2slug・2項目（`arno-nollen`=`Conceptual Art`、`oliver-musovik`=`Documentary`）。各項目は新JA素材§REL / 新EN素材§REL / 適用後JA§RELで完全一致0 / 0 / 0、旧項目に新素材の対応物なし。`nate-lowman` の `Pictures Generation` は条件不成立のためforceせず実在movementへ復元。最終builder dry-runは全7件`Would write 1 page(s)`、SKIPPED 0。
- **フィデリティ（実測・監督側で再測し一致）**：

  | slug | JA本文字数 | EN本文字数 | JA unique出典 | EN unique出典 | JA bytes | EN bytes |
  |---|---:|---:|---:|---:|---:|---:|
  | arno-nollen | 823→4,687 | 3,063→10,966 | 2→26 | 2→26 | 108,369→67,485 | 108,102→69,431 |
  | multiplicity | 4→5,133 | 12→11,467 | 0→28 | 0→28 | 101,890→68,488 | 100,260→69,480 |
  | nate-lowman | 670→4,418 | 2,580→10,424 | 2→27 | 2→27 | 108,009→67,539 | 107,677→68,801 |
  | ohio | 4→4,587 | 12→10,798 | 0→26 | 0→26 | 101,668→66,939 | 100,034→68,817 |
  | oliver-musovik | 749→4,710 | 3,496→11,446 | 3→28 | 3→28 | 109,568→68,590 | 110,426→70,584 |
  | paul-albert-leitner | 783→9,785 | 3,377→24,640 | 2→33 | 2→33 | 108,789→91,032 | 109,313→94,791 |
  | wangechi-mutu | 4→5,178 | 12→11,745 | 0→30 | 0→30 | 101,954→70,772 | 100,415→71,624 |
  | **合計** | **3,037→38,498** | **12,552→91,486** | **9→198** | **9→198** | **740,247→500,845** | **736,227→513,528** |

- **構造 / 正本**：全14ページでJSON-LD Personキー集合の減少0（JA `arno-nollen` 9→10・`multiplicity` 0→7の追加のみ、EN全7件不変）、div/section開閉一致（JA div 98〜105組 / EN div 150〜172組 / section 全件9組）、`rev` / `revision` / `revN` / `revision-*` の全語形残存0、本文二重ダッシュ0、dangling sup-ref 0。h3取りこぼし0（素材→適用後で6名8→8、`paul-albert-leitner` 18→18）。EN不可視要素はGA / canonical / hreflang / og:image / JSON-LDが全14ページでbefore=after。EN正本pages 306→306、変更キーは対象7slugのみ、`_meta`不変、entryキーは4件28→28 / 3件27→27で欠落0、対象外entry変更0。
- **§REL / SHA**：JA 10リンク / EN 10リンクをhrefごとにrepo root起点で実在確認し切れ0・張り忘れ0（監督側の独立実測でも内部リンク134本の切れ0）。0820素材14/14のSHA-256は作業前後一致（mismatch 0）。
- **検証**：`check_content_loss.py` EXIT 0、`preflight.py` EXIT 0、`check_en_entry.py` 全7件完走、全7slugのbuilder dry-runはSKIPPED 0、`git diff --check` OK。preflightのバッチ由来WARNは承認済み`data-nosnippet`減少6ページ（`multiplicity` / `ohio` / `wangechi-mutu` のJA 12→8・EN 9→8）のみで、これはprep-blockプレースホルダが実コンテンツへ置換された正当な減少。既存stale intentional-replacement宣言17件と既知`/colophon` 2件は対象外。禁止面（card-data / cards-archive / archive / eras / countries / movements / design / styles / new-design / scripts / sitemap）差分0、対象外写真家0、engine差分0。
- **監督側の独立実測（Codex報告の裏取り）**：Years確定後のtracked差分16件＝期待集合（公開HTML14 / EN正本1 / 本ログ1）と完全一致、素材SHA 14/14不変、本文字数・出典数・bytesを監督側の独立パーサで再測して全slug一致（減少slug0）、JSON-LDキー減少0、`rev`系トークン残存0、内部リンク134本で切れ0、EN正本 pages 306→306・変更キー7slugのみ・欠落0・`_meta`不変、不可視要素は`data-nosnippet`以外すべてbefore=after。
- **面**：公開HTML14、EN正本1、本ログ1のtracked 16ファイル。backup JA/EN各7とspec 7の未追跡21ファイルは未stage。commit / pushなし。
- **commit**：なし（Daisuke承認待ち）。
- **wall-time**：24分（Daisuke実測。7名バッチ＝**1名あたり3.4分**。既知記録は15名36分＝2.4分、14名48分＝3.4分、8名29分＝3.6分、12名44分＝3.7分、11名50分＝4.5分、10名55分＝5.5分、6名37分＝6.2分、4名27分＝6.8分、5名40分＝8.0分、7名60分＝8.6分で、**14名（3.43分）と同水準の3.43分**。注目点は規模で、これまで「同質素材なら大きめバッチほど1名あたりが速い」という按分傾向で説明していたが、**7名という小規模で、同じ7名の旧記録8.6分を2.5倍更新して14名タイに並んだ**。効いたのは §14 のキックオフ定型を初回プロンプトに全部載せたこと（3バッチ連続）で、**engine修正0・素材由来の停止0**。停止は1回のみ（`prep-block` orphan class FAIL＝素材ではなくスキャフォールド由来の新規事象。Codexは自力で押し通さず正しく停止し、監督が既存24ページの実測から標準解を出して往復1回で解決）。Years確定はDaisuke指示による追加ラウンドで、この24分には含まれない。
- **素材側の積み残し（次回ChatGPTプロンプトへ）**：`rev` クラスは0813に続き到来（`paul-albert-leitner` のみクリーン出力＝ハイライト抜きで出せることの実証）。**源流カットは引き続き素材出力時にハイライト抜きで出させること**。

## 2026-08-23 — paul-albert-leitner update追補
...
## 2026-08-25 — jean-luc-mylayne update（0825素材・単体）

- **種別 / 範囲**：update / 1slug（`jean-luc-mylayne`）。0825素材の日英本文・description・keywords・thesis・§REL・出典・作品リンクを反映。変更したtracked面はJA正本HTML 1 / EN正本JSON 1 / 生成EN HTML 1 / `scripts/intentional-replacements.json` 1 / 本ログ。hero眉 / Years（空） / Country `フランス` / Movement `—` / Period `1980–1990s` は§14 Cどおり既存維持。Amazon CTAはJA/ENとも0→0、新規検索URL0、ASIN解決なし。差分はhead 4行（description / og / twitter / JSON-LD description）とabstract〜出典＋sidebar `Updated` に限定され、本文外の構造・chromeは無変更。
- **bug / 手作業 / engine**：engine bug 0、engine改良0。手作業3系統。①JA §RELの `ジェフ・ウォール` が素材で裸テキストだったため `/photographers/wall.html` へリンク化（実在確認済）。②JA/EN素材とも `bernd-hilla-becher` という非実在slugを使っていたため、実ページの `becher` へ是正。③この②③に起因してEN §RELからBecher（非実在slugで落ちた）とJeff Wall（裸で落ちた）の2項目が欠落し日英非対称になっていたため、EN正本JSONの `site_directory_html` へEN素材原文のまま2項目を追加し、順序をJAと同じ Lempert → Becher → Jeff Wall → Kannisto にそろえてbuilder経由で再生成（EN HTMLの直接編集なし）。works ui-terms実検出は対象なし。EN builderのRelated削除SKIPは発生せず、`--force` 不使用。
- **フィデリティ**：JA本文4,615→10,072字、EN本文はページ全体で11,987→24,229字（本文抽出ベース）。unique出典はJA/ENとも23→31、sup-refはJA/ENとも33→80でdangling 0。§RELはJA 5→9項目（リンク2→4）/ EN 2→4（全リンク）、作品リンクはJA/ENとも4→7。bytesはJA 67,985→89,681 / EN 71,404→91,778。
- **意図的URL置換の宣言**：EN正本の外部リンクは32→41（純増+9）で、旧出典6件（aargauerkunsthaus / biennale-melle / chicagoreader / fadmagazine / independent.co.uk / lannan.org のexhibition grantページ）が消失。6件とも新JA素材・新EN素材のどちらにも存在せず、新素材31出典への総入れ替えに伴う正当な削除であることを実測したうえで、`scripts/intentional-replacements.json` へ6件を宣言（reason=新素材31出典への総入れ替えに伴う旧出典URLの意図的削除 / declared=2026-08-25）。既存宣言は不変。stale宣言WARNは17件で今回は未削除。
- **構造 / 正本 / 不可視要素**：JSON-LDキー減少0（JA `Person` 8→8、EN `WebPage` 8 / `WebSite` 3 / `Person` 9 / `Country` 2 / `BreadcrumbList` 3 / `ListItem`×3 がすべてbackupと同一）。不可視要素はJAがGA `G-2VRTV8BZEJ` 2→2 / canonical 1→1 / hreflang 3→3 / og:image 0→0 / JSON-LD 1→1、ENがGA 2→2 / canonical 1→1 / hreflang 3→3 / og:image 4→4 / JSON-LD 2→2。`data-nosnippet`はJA 8→8 / EN 7→7で減少なし。div/sectionはJA 103/103・9/9、EN 165/165・9/9。review marker / 二重ダッシュ / `prep-block` / dangling sup-refはいずれも0。EN正本pages 307→307、変更キーは `jean-luc-mylayne.html` のみ、entryキー28→28（増減0）、`_meta` 不変。
- **検索配線**：JA/ENとも runtime解決形 `input.ph-side-search__input:not([id*=mobile])` 各1で、ページ内の `ph-search-input-*` / `ph-search-suggestions-*` リテラルは自slug 4＋mobile 4の計8。参照実装 `ansel-adams` および適用前backupと同数・同形で、他slugの焼き込み0。`preflight.py` の `check_sidebar_search_wiring()` もEXIT 0。
...
## 2026-08-28 — ファビコン新規追加（Google検索結果アイコン / Opus監督・Codex実装）

- **種別 / 範囲**：other / サイト全体のファビコン新設。作業前調査で**916ページ全てに `<link rel="icon">` が存在せず、ルートにもアイコン画像が皆無**だと確認（Google検索結果がデフォルトの地球儀アイコンになっていた原因）。コンテンツ・本文・構造の変更は一切なし。
- **仕様 / 設計**：Daisuke添付のターゲット型マーク（赤地＋白同心リング＋中心点）。添付バイト列がディスクに残っていなかったため純図形としてベクター再構成。地色 `#D6432F` / リング `#F7F2EA` / 中心点 `#D8E64B`（ライムイエロー・Daisuke指示で白から変更）/ リング外径=一辺の62.5% / 線幅10.83% / 中心点24.58%、角丸なし・不透明。JA/ENで分けられるかを検討したが、Google公式仕様で *site* は **hostname 単位**であり `eyescosmos.com/en/` はサブディレクトリのため**検索結果のアイコンは分割不可**と確定（サブドメイン化は8/27のドメイン移行直後のためリスク超過と判断し不採用）。よってJA/EN共通の1アイコンとした。
- **実装 / 手作業点**：Codexへ委譲。環境に rsvg-convert / ImageMagick / inkscape が無いためSVG経由を捨て、**Pillow の ImageDraw で直接描画**（8倍スーパーサンプリング→LANCZOS縮小、リングは外径塗り→内径を地色で塗り戻す方式で線幅厳守）。新規6ファイルをルート直下に作成：`favicon.svg` / `favicon-48.png` / `favicon-96.png` / `favicon-192.png` / `apple-touch-icon.png`（180）/ `favicon.ico`（16・32・48内包）。作業スクリプトはscratchpadに置きリポジトリ内へ残していない。
- **サーフェス変更**：tracked 3面のみ＝`index.html` / `en/index.html`（各 +4行・削除0）＋本ログ。挿入位置は両ファイルとも `hreflang="x-default"` 行の直後で、`rel="icon"`（ico/svg/png96）＋`apple-touch-icon` の4行。**他ページ913枚には配線せず**、ルート `/favicon.ico` のブラウザ自動探索に委ねた（最小差分・生成系ページを触らないため）。
- **検証**：`git diff --stat` は `2 files changed, 8 insertions(+)` のみで削除0・対象外trackedの差分0。全PNG/ICOをPillowで画素実測し、角=`(214,67,47)`・中心/リング=`(247,242,234)`・リング内外の隙間が地色であることを4ファイルで確認。`favicon.ico` の内包サイズは `[(16,16),(32,32),(48,48)]`。**16px実物を16倍拡大して目視確認し、リングと中心点が分離して読めることを確認**（検索結果で効くのがこのサイズのため）。head参照4パスの実在確認OK、`git check-ignore` で6ファイルとも追跡可能を確認、sitemap/robotsの変更なし。`check_content_loss.py` EXIT 0、`preflight.py` EXIT 0（増分WARN 0。既出は0827バッチのstale intentional-replacement 8件と既知の `/colophon` のみ）。generate_*/build_* は一切実行せず、git add / commit / push / stash なし。
- **反映見込み**：Googleはトップページのheadのみを参照するため、`/` の再クロール後に反映（数日〜数週間）。
- **wall-time**：（Daisuke記入）

## 2026-08-28 — トップWebSite JSON-LD追加＋OGP画像刷新＋ファビコン配色統一（Opus監督・Codex実装）

- **種別 / 範囲**：other / トップ2枚（`index.html` / `en/index.html`）＋共通OGP画像＋ファビコン6ファイル。素材は Daisuke 提供の `OPG/ogp-export 2/`（`og-top.png` / `og-photographer.png` / `ogp-template.html` / `ogp.css` / README）。**素材HTMLの構造・デザインはサイトへ流用せず、PNG書き出しの版下としてのみ使用**（CLAUDE.md「渡された素材の扱い」準拠）。本文・構造の変更は0。
- **A. WebSite JSON-LD**：検索結果のサイト名が「GitHub」表示だった原因が、**トップ・JA全域にWebSiteノードが1つも存在しないこと**だと実測で特定（JA写真家ページのJSON-LD `@type` は `Person` のみ、`archive.html` もld+json 0、JA/ENトップともld+json 0）。`</head>` 直前に `WebSite` を各1件追加（各+10行・削除0）。値はJA `name=写真の座標` / `alternateName=Photo Coordinates` / `url=https://eyescosmos.com/` / `inLanguage=ja`、EN `name=Photo Coordinates` / `alternateName=写真の座標` / `url=https://eyescosmos.com/en/` / `inLanguage=en`。**ENの `name` / `url` は既存EN写真家ページの `isPartOf` 内WebSiteノードと完全一致させ、サイト内で名称が割れないようにした**。新規の命名は作っていない。
- **B. OGP画像**：素材 `og-top.png` は右下に `PHOTO-COORDINATES.JP` が焼き込まれており、READMEの埋め込み例も `https://photo-coordinates.jp/og/top.png` だった。**`dig` で NXDOMAIN（存在しないドメイン）と確認**したためプレースホルダと判断し、`ogp-template.html` の `data-edit="url"`（33行目）だけを `eyescosmos.com` に差し替えてヘッドレスChromeで `#og-top` を1200×630再書き出しし、`assets/ogp-default.png` へ上書き。**ファイル名は変更せず**、`.claude/worktrees/` の複製1368件を除いた**実参照471ページ**（en 388 / movements 35 / countries 33 / eras 11 / ルート4）が**HTMLのog:imageタグを1つも変更せずに一斉更新**され、旧 `eyescosmos.github.io` 焼き込み問題も同時に解消。フォント欠落検知のため**無改変レンダを素材と画素比較**（完全一致94.71% / 平均絶対誤差1.23/255）してGoogle Fonts（Noto Sans JP・JetBrains Mono）の読込を確認してから本番用を出力。URLテキスト幅は新143px（EYESCOSMOS.COM・14字）対 素材208px（20字）で字数比と整合し、旧URL残存0。
- **C. ファビコン配色統一**：素材 `ogp.css` の `:root` にブランドトークン（`--verm:#d6402e` / `--cream:#fafaf5` / `--lime:#ccff00`）が定義されており、同日先行commit `64a8242ed` のファビコン（`#D6432F` / `#F7F2EA` / `#D8E64B`）と不一致だった。**テンプレ右上の `.og__mark` がファビコンと同一図形（白リング＋ライム中心）＝元々ペアの設計**のため、Daisuke指示で3値ともトークン値へ統一し6ファイルを再生成。幾何は不変。
- **監査（消失・巻き戻り・巻き込み）**：`origin/main` 基準で差分9ファイル・**追加23行 / 削除3行**、削除3行は `favicon.svg` の色定義のみで全文列挙して確認。トップ2枚をorigin/mainと要素単位で突合し、GA `G-2VRTV8BZEJ` 2、canonical 1、hreflang 3、`og:` 10、`twitter:` 4、og:image 1、`rel="icon"` 3、apple-touch-icon 1、`data-nosnippet` 4、title 1、description 1が**全て非減少**、増加はld+json 0→1のみ。div 240/240・section 4/4・head/body/style 各1/1で開閉バランス不変、scriptは3→4（JSON-LD分）で開閉一致。`photographers/` `en/photographers/` `countries/` `en/countries/` `eras/` `movements/` `data/` `scripts/` `styles/` `new-design/` `card-data.json` `cards-archive.html` `archive.html` `sitemap*.xml` `robots.txt` `CNAME` は**差分0**。未追跡206件（backup/spec）は作業前と同一で1件もstageせず。
- **検証**：`check_content_loss.py` EXIT 0、`preflight.py` EXIT 0（増分WARN 0）、`git diff --check` EXIT 0。ファビコン4PNGは角`(214,64,46)`/中心`(204,255,0)`・alpha全面255、`favicon.ico` 内包`[(16,16),(32,32),(48,48)]`。OGPは1200×630 RGBでHTML宣言の`og:image:width/height`と一致。generate_* / build_* は一切未実行。
- **wall-time**：（Daisuke記入）

## 2026-08-29 — 0829写真家14名 update パイロット（anders-petersen / PHASE 1）

- **種別 / 範囲**：updateパイロット / 1slug（`anders-petersen`）。0829素材の日英本文・description・keywords・thesis・§REL・出典・作品リンクを反映。残り13slugは未着手。hero眉 / Years（空） / Country（JA=`スウェーデン`、EN=`Sweden`） / Movement `—` / Period `1970–1980s` はJA/ENともbackup一致。Amazonリンク・検索URLはJA/ENとも0→0、新規検索URL0、ASIN解決なし。
- **bug / 手作業 / engine**：engine変更0。手作業3系統。①旧JA §REFの手キュレーション2件（ICP / 1854）とグループ見出し「関連データベース・アーカイブ」が素材適用で落ちたため、backup原文のまま新素材2リンクの前へ復元。最終§REF href集合はJA/ENとも4件で完全一致し、旧2＋新2の欠落0。②旧EN出典Spot Home Gallery URL 1件が新JA/EN素材の§SRC各20 URLに0件であることを実測し、`intentional-replacements.json`へ1URL限定・declared=`2026-08-29`で宣言。③EN builderのRelated削除SKIP（`Documentary` / `Photojournalism`）は、各項目が新JA素材§REL・新EN素材§REL・適用後JA§RELに無く、旧項目に対応物も無いことを実測して§14 Bの3条件成立後、このslugだけ`--force`で生成。works ui-terms追加候補0。
- **フィデリティ**：JA本文849→3,901字 / EN本文2,222→9,193字、unique出典はJA/ENとも3→20、sup-refはJA5→31 / EN0→31、§RELはJA2→3（リンク2） / EN2→2（リンク2）、§REF hrefはJA/ENとも2→4、作品リンクはJA/ENとも0→3、bytesはJA107,975→64,851 / EN105,571→66,876。dangling sup-refはJA/ENとも0。
- **構造 / 正本 / 不可視要素**：JSON-LDキー減少0（JA Person 8→8、EN WebPage 8 / WebSite 3 / Person 9 / Country 2 / BreadcrumbList 3 / ListItem 4は全て不変）。GA `G-2VRTV8BZEJ`はJA/ENとも2→2、canonical 1→1、hreflang 3→3、JSON-LD scriptはJA1→1 / EN2→2、og:imageはJA0→0 / EN1→1。`data-nosnippet`はJA9→8 / EN8→7（prep-block置換による既知の正当減少）。タグ開閉はJA div92/92・section9/9、EN div132/132・section9/9。review class / CSS selector / 二重ダッシュ / `prep-block`はJA/ENとも0。EN正本pages 307→307、変更キーは`anders-petersen.html`のみ、entryキー28→28・欠落0、`_meta`不変。
- **検索配線 / §REL / SHA**：JA/ENともruntime解決形`input.ph-side-search__input:not([id*=mobile])`各1、他slug固定値0。§RELリンクはJA2＋EN2の全4件をrepo root起点で実在確認し切れ0。裸項目Christer Strömholmは`card-data.json`に対応カードがなく実ページも無いため正当、張り忘れ0。素材28/28のSHA-256は作業前後一致（合成SHA `78a1d55a83371e5d6e8a8e134cecf59c52b5dd1839d371714845798433cfb0eb`、mismatch 0）。
- **検証 / 面**：`check_en_entry.py anders-petersen` EXIT 0、`check_content_loss.py` EXIT 0、`preflight.py` EXIT 0、builder最終通常dry-runは`Would write 1 page(s)`・SKIPPED 0、`git diff --check` EXIT 0。preflight baselineからの増分WARNは`data-nosnippet`減少のJA/EN各1行のみ。tracked面はJA正本1 / EN正本JSON1 / 生成EN1 / intentional-replacements1 / 本ログ1。禁止面・対象外写真家ページ差分0。backup JA/EN各1とspec 1は未追跡・未stage。git add / commit / push / stash / checkout / restoreなし。PHASE 1で停止し、監督監査待ち。
- **wall-time**：PHASE 2と合算（下記）

## 2026-08-29 — 0829写真家14名 update バッチ完了（残り13slug / PHASE 2）

- **種別 / 範囲**：update 13名（`axel-hutte` / `dorothy-bohm` / `erwin-wurm` / `izis` / `jessica-eaton` / `kate-steciw` / `louis-faurer` / `manfred-willmann` / `nikki-s-lee` / `norihiko-matsumoto` / `noriko-hayashi` / `sara-vanderbeek` / `william-christenberry`）。PHASE 1の`anders-petersen`と合わせて対象14名を完了。JA HTML正本、EN JSON正本、builder生成ENへ0829素材の本文・description・keywords・thesis・§REL・出典・作品リンクを反映。hero眉 / Years / Country / Movement / Period は13/13・JA/ENともbackup一致。Amazon検索URLは全26面0→0、新規検索URL0、ASIN解決なし。
- **§REL carry-forward / 張り忘れ**：13名全員で旧JA項目−新素材項目をcard-data日本語名とmovement実ファイルで判定。旧only 16件は16/16が実ページを持つ手キュレーション項目だったため削除0・旧注記原文のまま和集合化（Dorothy=`Ed van der Elsken` / `Street Photography` / `Color Photography`、Izis=`Brassaï` / `Henri Cartier-Bresson` / `Documentary`、Jessica=`Modernism`、Louis=`Diane Arbus` / `Street Photography` / `Modernism`、Norihiko=`Documentary`、Noriko=`Photojournalism` / `Documentary`、Sara=`Walker Evans`、William=`Color Photography` / `Documentary`）。新素材の裸項目10件は、実ページあり2件（Dorothy=`André Kertész`、Louis=`Robert Frank`=`robertfrank`）をリンク化、実ページなし8件は裸を維持。最終§RELはJA62項目（リンク54）/ EN54項目（リンク54）、全108リンク実在・張り忘れ0。EN復元16注記は旧正本`related_annotations`と16/16 byte値一致。`ALLOW_REL_REDUCTION`不使用、Related削除SKIP 0、builder `--force`不使用。
- **§REF / 出典 / 辞書**：旧JA §REFの手キュレーション31リンクが13/13で素材適用時に落ちたため、各backupの旧ラベル・URL・グループ見出しを原文のまま新素材リンク前へ復元。最終href集合はJA/EN完全一致（Axel 10 / Dorothy 4 / Erwin 6 / Izis 4 / Jessica 5 / Kate 4 / Louis 5 / Manfred 6 / Nikki 5 / Norihiko 5 / Noriko 3 / Sara 5 / William 5、計67）、旧backup欠落0・新素材欠落0。旧出典URL4件は新JA/EN素材§SRC双方0件を実測しintentional replacement宣言（Izis 1 / Kate 2 / Sara 1、declared=`2026-08-29`）。works ui-terms 4件をURL照合から追加。複合Countryの既存EN表示維持のため既存辞書経路へ`リトアニア= Lithuania`を1件追加し、Dorothy / Izisを再生成。
- **フィデリティ**：

  | slug | JA本文字数 | EN本文字数 | unique出典 JA/EN | sup-ref JA/EN | §REL JA/EN（リンク） | §REF href JA/EN | 作品 JA/EN | bytes JA / EN |
  |---|---:|---:|---:|---:|---:|---:|---:|---:|
  | axel-hutte | 963→5,859 | 4,100→15,352 | 7→25 / 7→25 | 7→38 / 45→38 | 5→5 (5) / 5→5 (5) | 7→10 / 7→10 | 7→3 / 7→3 | 115,218→76,000 / 115,850→77,311 |
  | dorothy-bohm | 900→4,572 | 2,198→10,340 | 3→25 / 3→25 | 21→35 / 19→35 | 4→6 (5) / 4→5 (5) | 2→4 / 2→4 | 2→3 / 2→3 | 110,774→68,663 / 108,314→71,173 |
  | erwin-wurm | 935→5,155 | 3,388→12,956 | 3→25 / 3→25 | 6→35 / 40→35 | 1→4 (4) / 1→4 (4) | 3→6 / 3→6 | 3→3 / 3→3 | 110,172→70,711 / 110,590→72,697 |
  | izis | 984→4,481 | 2,055→10,724 | 3→23 / 3→23 | 18→36 / 16→36 | 3→6 (4) / 3→4 (4) | 2→4 / 2→4 | 2→3 / 2→3 | 109,934→68,199 / 107,595→70,329 |
  | jessica-eaton | 828→5,053 | 1,921→12,280 | 4→24 / 4→24 | 5→36 / 5→36 | 1→5 (5) / 1→5 (5) | 2→5 / 2→5 | 0→3 / 0→3 | 107,186→69,207 / 105,546→71,971 |
  | kate-steciw | 858→5,457 | 1,947→13,413 | 5→26 / 5→26 | 5→35 / 5→35 | 1→4 (4) / 1→4 (4) | 2→4 / 2→4 | 0→3 / 0→3 | 108,220→70,657 / 105,853→73,053 |
  | louis-faurer | 984→4,283 | 2,179→9,752 | 3→24 / 3→24 | 22→34 / 22→34 | 5→6 (6) / 5→6 (6) | 3→5 / 3→5 | 3→3 / 3→3 | 111,091→67,226 / 108,656→69,710 |
  | manfred-willmann | 677→6,484 | 3,269→15,417 | 2→29 / 2→29 | 5→39 / 26→39 | 1→4 (4) / 1→4 (4) | 2→6 / 2→6 | 2→3 / 2→3 | 108,063→74,526 / 108,636→77,229 |
  | nikki-s-lee | 921→5,285 | 3,310→12,797 | 3→24 / 3→24 | 5→33 / 36→33 | 2→3 (3) / 2→3 (3) | 3→5 / 3→5 | 3→3 / 3→3 | 110,521→70,489 / 110,800→71,728 |
  | norihiko-matsumoto | 785→4,435 | 3,202→12,703 | 3→28 / 3→28 | 9→43 / 41→43 | 1→4 (3) / 1→3 (3) | 3→5 / 3→5 | 0→3 / 0→3 | 107,403→69,992 / 108,910→74,203 |
  | noriko-hayashi | 925→4,629 | 1,994→11,659 | 4→30 / 4→30 | 5→39 / 5→39 | 2→5 (2) / 2→2 (2) | 2→3 / 2→3 | 0→3 / 0→3 | 107,869→70,060 / 106,141→72,550 |
  | sara-vanderbeek | 904→5,984 | 1,934→15,432 | 4→29 / 4→29 | 5→41 / 5→41 | 2→5 (5) / 2→5 (5) | 2→5 / 2→5 | 0→3 / 0→3 | 108,335→73,736 / 106,248→77,317 |
  | william-christenberry | 1,028→4,626 | 2,139→10,855 | 3→22 / 3→22 | 6→34 / 0→34 | 3→5 (4) / 3→4 (4) | 3→5 / 3→5 | 0→3 / 0→3 | 109,137→68,657 / 106,259→70,402 |
...
### 2026-08-31 — カード解説文3件の文体是正（taiji-matsue / fabian-marti / gabriel-orozco・other）
- **背景**：0830素材12名 update バッチはカード面が禁止面のため触っていなかった。実測で12名中9名は現行文面で問題なし、残る3名だけが旧文面（松江＝生年カッコ書き＋3文、マルティ／オロスコ＝旧スタブ文）だった。
- **やったこと**：`archive.html` のカード解説3件を、同バッチの他9名と同じ形（1文・生年なし・名前始まり）へ差し替え。新文面は各 `photographers/<slug>.html` の meta description と thesis からの要約で、新規調査・追記なし。`data/photographer-essay-overrides.js` の `taiji-matsue` は `leadJa` / `leadEn` を更新（`textJa` / `textEn` は死にデータのため不変）。`en/archive.html` は `python3 scripts/build_archive_en.py` で再生成。
- **差分範囲**：当初 `archive.html` / `en/archive.html` / `overrides.js` の3ファイルに限定して作業したが、**これは誤り**だった。Daisuke の運用ルール「カードを追加するときはアーカイブからコピーする」が指すコピー元は `cards-archive.html` / `card-data.json`（CLAUDE.md 禁止事項8）であり、`archive.html` はそれとは別の並列コピー（`generate_archive_pages.py` のガード文＝「現行アーカイブの正本は archive.html（直接編集）」）。3ファイルだけ直した段階では**コピー元が旧文面のまま**で、以後のカード追加で旧文面が拡散する状態だった。指摘を受けて `cards-archive.html` / `card-data.json` も同文面へ更新し、3面（archive / cards-archive / card-data）を一致させた。
- **教訓（スコープ確認の言い回し）**：スコープを問うときに「正本」という抽象語を使ったため、「あなたがコピー元にしているファイル」という実務上の意味が伝わらず、誤った選択を招いた。**面の名前とその面が実際に何に読まれるかを具体名で提示すること**。
- **カード文面の伝播経路（実測）**：①トップページの検索・フィルタ結果は `cards-archive.html` を fetch して描画（`index.html:3097` / `:3174`。`card-data.json` の `ledeJa` は取得失敗時のフォールバック `index.html:3291`）。②国別・年代ページはビルド時に `card-data.json` の `ledeJa` を焼き込む（`generate_country_pages.py:107` / `add_missing_era_cards.py:107`）。③`archive.html` はどこからも生成されない直接編集面。国別ページ本体の `fetch('/card-data.json')` はサイドバー検索用で lede を読まない。
- **従属面への展開（同日・第2フェーズ）**：`countries/japan.html` · `switzerland.html` · `mexico.html`、`eras/1990.html` · `eras/2000.html`、`movements/スティルライフ.html` と各EN版の計12ファイルを正本へ揃えた。JAは `card-data.json` の `ledeJa`、ENは `en/archive.html` のカード lede が実効ソース（`build_taxonomy_en.py` は `load_en_archive_cards()` で `en/archive.html` から取り、`FALLBACK_LEDE_EN` に3名は不在。`generate_country_pages_en.py` も EN カードを `en/archive.html` から取る）。`data/taxonomy-en-content.json` にも旧文面が残るが、`sections/Photographers` は `build_taxonomy_en.py:1545` の `skip_keys` に入っていて描画されない休眠データのため未更新。
- **bug 3件目（生成スクリプトのコロフォン退行）— 実測で発見**：スコープ付き再生成を試したところ、`generate_country_pages_en.py --country switzerland` が footer の `<a href="/en/colophon">Colophon</a>` を**削除**した。原因は 2026-08-30 のコロフォン新設に生成側テンプレが未追随。同型を `generate_country_pages.py`（JA・コロフォンリンク欠落）と `build_taxonomy_en.py`（`translate_footer()` が `>コロフォン<` の**文字だけ**訳し `href="/colophon"` を残す＝ENページからJAコロフォンへリンク）でも確認。3本とも是正した。**preflight が推奨している再生成コマンド自体が退行を起こす状態だった**ため、この修正なしにこれらのページは再生成できない。
- **再生成の副次差分（監査済み・全て正本へのドリフト是正）**：カード集合・footer・name/meta/channel は全12ファイルで無変化。lede は計41件が更新され、うち14件が今回の対象、残りは 0830 バッチ以降 EN taxonomy/country が再生成されていなかった持ち越し分。加えて**タグ3件**が動いた — `gabriel-orozco`（JA/EN国別: コンセプチュアル→スティルライフ）と `diana-scheunemann`（countries/switzerland.html: コンセプチュアルアート→インティメイト・ライフ）。いずれも `card-data.json` / `archive.html` が既に持っていた値で、古い国別ページ側のドリフトが是正されたもの。**`diana-scheunemann` は依頼対象外の写真家**なので、この1件は要確認事項として残す。
- **preflight WARN 6件（EN country ×3 / EN eras ×2 / EN movement ×1）の判定**：**真陽性ではない**。根拠は憶測ではなく実測の順序で確定した ——①推奨コマンドを実際に実行し、いまディスク上のファイルは生成物そのもの、②それでも WARN が消えない＝生成物との乖離を見ているのではない、③判定コード（`preflight.py:698-704` 他）は「EN HTML が変わったのに `data/country-pages.json` / `data/taxonomy-en-content.json` の該当エントリが変わっていない」だけで発火する、④`data/country-pages.json` のエントリは `slug/codes/nameJa/nameEn/lead/updated` のみで**カード lede を持たない**。つまりこの WARN は編集で満たすことが原理的にできない。**ただし 2026-08-31 の `[EN archive]` WARN は真陽性だった**（`card-data.json` を正しく指していた）。WARN の指す正本が当該変更を実際に保持しているかを毎回確認すること。
- **bug 2件（いずれも既存スクリプトの陳腐化。今回の作業で踏んで是正）**：
  1. `build_archive_en.py` の chrome 置換表が `<a href="#">コロフォン</a>` を要求しており、2026-08-30 のコロフォン新設で `/colophon` へ変わった archive.html に一致せず **SystemExit で再生成不能**になっていた。置換表を実文字列へ更新。
  2. 同スクリプトの `build_movement_slug_map()` が `url=https://eyescosmos.github.io/en/movements/...` を決め打ちしており、2026-08-27 の独自ドメイン移行後の `eyescosmos.com` スタブ に一致せず**マップが空**になっていた。この状態で再生成すると en/archive.html の運動リンクが `/en/movements/ピクトリアリズム.html` 等の日本語ファイル名（リダイレクトシム）へ退行する。正規表現を `(?:com|github\.io)` へ一般化して是正（マップ31件を復元）。
- **判明したこと（対応不要・要記憶）**：`parse_overrides_lead_en()` は `leadEn:`（無クォートキー）だけを拾う。`overrides.js` は末尾34件が `"leadEn":`（クォートキー）の JSON 形式で書かれており、**この34件の `leadEn` はビルダーに読まれない**。`taiji-matsue` はこの34件側なので、今回の `leadEn` 更新は en/archive.html の表示には効いていない。EN カード文面の実効的な出所は `en/photographers/<slug>.html` の冒頭段落（EN正本）で、これは他9名（alec-soth 等）と同じ経路。
...
## 2026-09-05 — 0905素材の12名を新規追加（idx 342–353・種別=new・Opus監督 / Codex実装）

- **範囲**：new×12（alfred-rosling / edward-l-wilson / ernest-farmer / frank-sutcliffe / george-davison / henry-hamilton-bennett / horsley-hinton / james-craig-annan / john-benjamin-stone / paul-martin / tempest-anderson / valentine-blanchard）。JA/ENリーフ24枚を scaffold-inject / EN正本JSON経由で追加。card-data・星bin・supplement、archive 3面、年代3組、国2組、運動3組、index JA/EN、既存逆リンクJA/EN、sitemap 2面へ反映。ローカル専用 `new-design/cards-archive.html` もツール既定で更新。commit / pushなし。
- **手作業点 / bug / engine**：手作業7系統。①素材と確定taxonomyからspec 12本を作成、②素材のリスト型／カード型§RELをJA標準 `ph-rel-*` とEN正本 `site_directory_html` / `related_annotations` へ移植、③12名登録後に相互参照を再解決しFrank→Hinton等をリンク化、④Wellingtonの逆向き裸参照2件をJA/ENでリンク化、⑤運動6面へ対象カードを最小挿入、⑥素材由来レビュー／壊れclassを既存標準へ正規化、⑦importerが取り込めなかった§REFを復元。bug 4件＝§REL二形式の抽出漏れ、§REFの抽出漏れ、`inline-` / `inline-link` / review class残存、Roslingの不確定没年に対するJSON-LD生年ガード発火。engine/CSS変更0。ボトルネックは§REL・§REFの正本形式への手移植と逆リンク照合。
- **Rosling / 表記**：表示は `1802–1880s?` をhero・dt/dd・side-meta・card-dataの4面で維持。NGA / Getty / BLで一致する `birthDate: "1802"` のみJA/EN Personへ追加し、資料が割れる `deathDate` は省略。George Davisonは今回の生成物・正本・全従属面で「ジョージ・デイヴィソン」に統一。
- **正本 / データ**：card-data写真家341→353。EN正本pages 354→366、追加12・削除0・`_meta`不変。既存変更は `james-booker-blakemore-wellington.html` の `site_directory_html` / `related_annotations` へのhref 2件追加のみ。星データは12名全員のslugあり。`link_country_keywords.py` は最後に1回だけ実行（722枚処理・228枚変更）し、対象外tracked 203枚を `git restore`、新規24枚はuntrackedのため保持。
- **フィデリティ**：素材＝生成で本文h3 JA/EN各123→123、`id="cite-"` 各330→330、`class="ph-work` 各30→30、`ph-kw` 各78→78、§REF href各19→19、ミスマッチ0。raw h3はリスト型§RELの見出し2件が標準 `ph-rel-label` 化するページのみ-2で正常。Amazon `/s?k=` 新規0。
- **分類面**：UK +10、US +2。Pictorialismはカード12 / meta 10（着手前）→16 / 14（完了後）で既存drift 2を維持。Naturalistic Photography 1→3、Street Photography 12→13。運動ページの追加前既存カードはJA/ENともbyte変更0。
- **構造 / 検証**：24枚すべてdiv/section開閉一致、検索runtime解決形24/24・固定JS文字列0、AI開示24/24、§RELリンク切れ0・登録済み裸名0。`essay-p` / `prep-block`実体 / review class / visible ASCII二重ダッシュは0。未知class 0、CSS追加0。EN不可視要素は全12枚でGA 2 / canonical 1 / hreflang 3 / JSON-LD 3。sitemap 854→878、追加24・削除0・backup/worktree URL 0。素材24枚のSHA-256前後一致。
- **検査結果**：`check_new_photographer.py` / `check_en_entry.py` を12名全員で実行。`build_photographers_en.py --slug frank-sutcliffe --dry-run` はSKIPPED 0。`check_photographer_link_integrity.py`、`sync_card_counts.py --check`、`check_content_loss.py`、`preflight.py` はすべてEXIT 0・HARD 0。preflight WARNはEN分類面の「直接編集疑い」8件、既存Pictorialism drift、既存カードtag91枚、既存stale宣言のみ。
...
## 2026-09-12 — 0911 SEO選定バッチ10名 update（JA全面置換＋EN全面更新・Opus監督 / Codex実装）

- **範囲**：update×10。`richard-avedon` / `hiroshi-sugimoto` / `moriyama` / `marey` / `muybridge` / `nadar` / `tokuko-ushioda` / `yuki-tawada` / `jp-鹿島清兵衛`（EN は `kajima-seibei`）/ `sibylle-bergemann`。新規追加なしのため card-data・archive・sitemap・星bin・年代・国・運動の各面は変更0。commit / push なし。wall-time は Daisuke 記入。
- **JA の実測**：本文は10/10で増加（合計 52,653→78,386字）。出典は 188→336。h3 は 93→137。`jp-鹿島清兵衛` 3,012→6,046字、`nadar` 1,602→5,962字が最大の伸び。差分は10ファイルで +716/−266行（素材に「HTMLを整形し直さない」を指示した結果、全面再整形は0）。
- **素材の欠落を復元した点（5系統）**：① §REF の「関連データベース・アーカイブ」ブロックが4ページで丸ごと欠落（avedon 7 / kajima 6 / tawada 4 / ushioda 4 リンク）→ 旧ページから復元。② `yuki-tawada` の写真集『Her smoke rose up forever』が別書籍へ差し替わっていた→両方併記。③ `richard-avedon` の thesis の器が `div`→`p`（サイト396ページが `p`）。④ `sibylle-bergemann` の §REL の相対 href を絶対パス化（4本）。⑤ 本文の未リンク言及5件をリンク化（sugimoto→tillmans、moriyama→takeji-iwamiya / tomatsu / robertfrank、avedon→martin-munkacsi）。moriyama の `岩宮武二` は旧ページに存在したリンクが素材で失われていた。
- **§WORKS の扱い**：5ページで計9本が入れ替わっていたが、作家インデックス→個別作品ページ、PDF→展覧会ページと具体化する差し替えだったため素材を採用（消失ではない）。
- **ガード改修（Daisuke 承認済み）**：`sibylle-bergemann` の素材がリーフ型仕様どおり独自セクション §ARCHIVES を削除（リンク5本は §WORKS へ統合済み）。セクション数減少は宣言で抑止できない設計だったため、`intentional-replacements.json` に `{slug, section}` 形式を追加し、`preflight.py` 側で **baseline に当該ラベルが実在し作業ツリーに無いことを `git show` で実地確認**したうえで、確認できた宣言数が減少数以上のときだけ除外するようにした。嘘の宣言（残存セクションを指定）が弾かれることを自己テストで確認。あわせて `check_en_content_loss` の JSON セクション数減少を、**描画後のページで節が減っていない場合は WARN** に落とした（旧 harvest が §REL を本文節として取り込んでいた個体の偽陽性。`hiroshi-sugimoto` 5→4 が該当、描画は 8→8）。
- **EN の実測**：9名は正本JSON（`lead_html` / `sections[]` / `thesis_html` / `sources_html` / `cite_ids` / `supref_ids` / `related_annotations`）経由で再生成、`sibylle-bergemann` は正本JSON未登録のため EN HTML を手編集。EN本文は合計 178,400字。**JA と EN の出典番号は10/10で完全一致**（43/39/32/32/34/28/31/35/24/38）。宙に浮いた参照・未参照の出典は全ページ0。
- **Codex 実装の実測**：翻訳は Codex（gpt-5系 CLI）に1名1プロセスで委譲。10名で所要 44分（1名あたり 205〜455秒）、パイロット `nadar` のトークンは 46,505。入力は JA ページから lead / thesis / 節 / 出典 / §REL だけを抽出した 29〜50KB の JSON に縮小。出力は12項目の機械検証（出典番号とURLの一致、宙に浮いた参照、JAパス混入、節数、器のクラス、h3 の id、関連欄のキーが実在EN ページか、sup-ref 書式）を通してから正本へ書き込む方式にし、**10/10が初回で合格**。
- **Codex を動かすまでに踏んだ罠**：MCP サーバーは CONNECTION_CLOSED。CLI も `~/.codex/config.toml` が ChatGPT デスクトップアプリ管理下になっており、切り分けで `model = "gpt-5.6-sol"`・`mcp_servers.node_repl`（startup_timeout 120秒）・`[agents]` の**3つすべてが単独で CLI を止める**ことを確認。`auth.json` だけをコピーし `model_reasoning_effort` 1行のみの CODEX_HOME を別に作って回避。さらに `codex exec` は**標準入力を閉じないと入力待ちで無限に止まる**（この事故で約2時間空転・`< /dev/null` と15分の watchdog を付けて解消）。
- **EN 側で追加で直した点**：`data/photographers-en-ui-terms.json` の `terms` に `ファッション写真` / `ファッション写真 / ポートレート` を追加（avedon のキーワード chip が未翻訳警告を出していた。トップレベルではなく `terms` 配下が正しい位置）。`hiroshi-sugimoto` は EN 固有の手書き FIG マーカー4件をビルダーが保護して SKIP したため、翻訳後の該当4見出しへ `FIG. 01`〜`FIG. 04` を付け直して解消。`richard-avedon` の EN §01 は先頭 h3 を外した（`check_photographer_link_integrity` が Biography を人名で始めることを要求。旧版も §01 に見出しなし）。`sibylle-bergemann` は素材の一言解説が名前込みだったため §REL で名前が二重になり、リード部を除去。
...
### 素材の事実の食い違い＝決着（2026-09-14 Daisuke 確認）
`stieglitz` の Met 作品ID 267836 について、旧ページは《The Terminal》、新素材は《The Steerage》としていた。
**Daisuke が Met 公式で確認し、新素材が正・旧ページが誤りと確定**：
267836 = Alfred Stieglitz, *The Steerage*, 1907（Photogravure・Object Number 33.43.419）。
《The Terminal》は別IDで、269300 や 269458 がそれにあたる。
作業時に素材を採用していたため**公開ページの修正は不要**だった（§WORKS のチップも §SRC の出典表記も
JA/EN とも 269300=The Terminal / 267836=The Steerage で一貫している）。

**教訓**：Met は同一写真の別プリント・別収蔵品に複数のIDを持つ（The Terminal に 269300 / 269458、
The Steerage に 267836 / 270061 / 269281 など）。**旧ページのIDと新素材のIDが違っても、それだけでは
どちらかが誤りとは言えない。誤りは「同じIDに違う作品名を当てている」場合だけ**。今回は後者だった。
metmuseum.org は HTTP 429 を返すことがあり、作業中の自動確認はあてにできない。
...
## 2026-09-16 — EN 本文の「日本語版の名残」を一掃（種別=表示修正・Opus監督 / Codex実装）

- **範囲**：`en/photographers/*.html` **76枚**。JA は変更0。`en/movements` / `en/eras` は本文に和文記号0で対象外。
  wall-time は Daisuke 記入。
- **依頼**：前の出典番号バッチの検証中に見つけた既存バックログ2件（EN本文のCJK記号残存・段落頭の孤立ピリオド）を
  「まとめて直す」。加えて**素材側で直っていないときに着手前に気づけるチェック**を入れる。
- **★調べて分かった前提**：EN の作品名の標準表記は **`<i>…</i>`（実測1,975件/284ページ）**で、
  `《》`『』は JA 表記の漏れ（150件/36ページ）。**出典欄（`ph-sources`）の日本語資料名と、
  EN運動ページのカード内日本語人名（`＝` `＆` `／` `々`）は正しい表記**なので触らない。
  ここを切り分けずに一括置換すると出典が壊れる。
- **適用した15規則と件数（すべて期待値と一致）**
  R1 `</sup>。` の句点をピリオドにして sup の前へ 62 / R2 段落頭の孤立ピリオド削除 8 /
  R3 余分な `。` 削除 2 / R4 `《X》`→`<i>X</i>` 133 / R5 `《<a>X</a>》`→`<a><i>X</i></a>` 11 /
  R6 閉じない `《` 6 / R7 `『』`→`<i>` 15 / R8 `（）`→`()` 3 / R9 `、`→`,` 1 /
  R10 works-note の日本語→EN標準文面 3 / R11 `</a>`+英字にスペース 45（複数形 `</a>s` 1件は除外）/
  R12 `word.Word`→`word. Word` 57 / R13 `》《`→`</i> <i>` 2 / R14 `》／`→`</i> / ` 1 /
  R15 `英数字《`→スペース挿入 1。
- **日本語のまま残したもの**：asako-narahashi の展覧会名「距離の不在：写真の現在」（本文に2回）、
  yurie-nagashima の作品名「縫うこと、着ること、語ること。」と展示名（末尾 `？`）。
  EN本文に残る全角記号は**この10個だけ**（「2 」2 ：2 、2 。1 ？1）。`《》『』（）` は0。
- **検査**：① `href` の列がファイル単位で完全一致（リンクを1本も失っていない）
  ② `#cite-N` の列が完全一致 ③ `ph-sources` がバイト一致 ④ 本文の文字数が97%以上
  ⑤ 15規則それぞれの適用件数が期待値と一致 ⑥ 冪等 ⑦ `check_content_loss.py` / `preflight.py` とも EXIT 0。
- **監督側の手戻り1回**：期待する最終状態で `「」` を各1個と書いたが、同じ展覧会名が本文に2回出るので各2個。
  Codex が書き込み前に停止して指摘（4回連続で「Codex が止まったら監督のブリーフが間違っている」）。

### 入れたガード2本（どちらも実測グリーン・発火確認済み）

- **`preflight.check_en_prose_hygiene()`＝HARD**（4分岐: 和文記号 / 段落頭の `. ` /
  `</a>`+英字 / `word.Word`）。対象は EN 3ディレクトリの `<main>` から `ph-sources` と
  `pc-*` カードを除いた本文。許容する日本語記号は `EN_PROSE_FULLWIDTH_ALLOWED` に明記。
  talbot の EN を故意に壊して4分岐すべてHARD、復元で0件を確認。
- **`import_chatgpt_photographer.py --precheck` の `[STYLE] 表記チェック`**＝**素材のまま着手前に見る**。
  出典番号の位置 / Abstract・thesis 混入（JA・EN 両方）＋ EN の和文記号・段落頭 `. `・空白抜け。
