...
> # ✅ 完了（2026-09-16）— **このファイルはもう読まなくてよい**
>
> 0915素材の新規6名（`yasuhiro-ishimoto` / `eve-arnold` / `guy-bourdin` / `inge-morath` /
> `marc-riboud` / `shigeichi-nagano`・idx 403–408）を追加し、**§13.10 B の初回検証は通過＝新規追加は
> 通常運用**。push は `cfa465d47`（本体）/ `c70805101`（ログ）。実測は `docs/importer-run-log.md` 2026-09-16。
>
> **以降の進め方**
> - **新規追加** … `docs/importer-scaffold-inject-spec.md` §14（キックオフ定型）だけ見ればよい。
>   入口は `--slug <slug> --ja JA.html --en EN.html --apply`（spec は `scripts/<slug>-spec.json` を自動探索。
>   **下の §3 の旧記述より、この1行が正**）。手補完4種（JA title / og:image / JSON-LD / §REL 書式）は
>   2026-09-16 の engine 修正（`e4cf03900`）で自動化済み＝ブリーフに書かない。
> - **既存ページの update** … **移行後の初回だけ** `docs/en-html-canon-migration.md` §13.10 の **A**（6項目）を
>   パイロット1名で回す。2回目以降は通常運用。
>
> 以下は 0915 バッチ当時の記録。**手順としては参照しない**（§3 の入口と §0 の「本番未検証」は古い）。
...
通常モードは **scaffold-inject 経路に一本化済み**。`--spec` を省略すると
`scripts/<slug>-spec.json` を探索し、無ければ書き込まず非0終了する。素材HTMLを素通しで公開しない。
`--render-ja` は一時出力での確認用、`--render-en` は JA が既にある場合の EN 単独追加用として残る。

```bash
python3 scripts/import_chatgpt_photographer.py --slug <slug> --ja JA.html --en EN.html --spec scripts/<slug>-spec.json --apply
```

JA を scaffold-inject で生成してから EN を描く。spec が既定位置なら `--spec` は省略可。
**EN 出力先が既に存在すれば `--force` でも拒否される。**
JA が既にあり EN だけ足すときは `--slug <slug> --render-en EN.html --apply` を使う。
