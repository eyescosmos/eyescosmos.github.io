...
- **既知WARN（ブロックしない）**：EN 面に和文句点「。」+ 出典番号が **62件** 残っている。
  これは *EN 本文に CJK 記号が残っている* という別バックログ（`。`73 / `《》`152 / `「」`28 /
  `『』`24 / `、`18 / `（）`10、EN写真家37ページ）。`SUPREF_EN_KUTEN_BASELINE = 62` を
  **超えたら HARD**。EN を直したらこの定数を下げる。
