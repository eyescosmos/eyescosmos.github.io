...
- 本文中の出典注は、1文につき最大1つまでにする。
- `<sup class="sup-ref">` を連続させない。`*1*2*3` や `[*1][*2][*3]` のような連番注は読みにくいため禁止。
- 1段落の末尾に複数出典をまとめて置かず、根拠が必要な文ごとに最も対応する出典を1つだけ置く。
- 複数の出典が必要な主張は、文を分けるか、より代表的な出典を1つ選ぶ。
- AIの引用・AIの主観・出典にない評価は本文に入れない。
