...
- **JA 写真家ページの SEO 穴検知 WARN（2026-06-19 追加）** — `check_ja_seo_holes()` が触った
  ...
  - **新規 JA 写真家ページを作るときの最善手：完成済みページ（例 `photographers/winogrand.html`）を丸ごと
    コピーして名前・本文だけ差し替える**。これで canonical/hreflang/OGP/description/JSON-LD/nosnippet/GA が
    最初から入る。ゼロから組むと GA 欠落＝HARD ブロック、残り6項目＝上記 WARN になる。
...
- **`scripts/add_photographer.py` ＋ `scripts/photographer-spec.example.json`** — 新規写真家を
  ...
  チェックリスト（ページ作成＝winogrand.html コピー／本文・thesis・§REL・cite／EN 正本 JSON／
  リンク後処理）を出す**ので、それに従えば 1 パスで作れる。
- **`scripts/check_new_photographer.py --slug <slug>`** — 新規／触った写真家ページの**完成検査**
  （構造健全さ＋決定論 cite 整合＋JSON-LD 実体準拠＝JA は Person を要求）。slug は通称可。
  `--strict-new` で不足を一部 HARD 化、`--all` で全ページ可視化。`check_photographer_link_integrity.py`
  とは非重複（曖昧判定は持ち込まない）。preflight にも touched-only で軽量統合済み
  （明確な破損だけ HARD・完成度不足は WARN）。**新規ページを作ったら必ずこれを通す**。
