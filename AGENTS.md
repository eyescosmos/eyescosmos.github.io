...
- `scripts/preflight.py` と `.githooks/pre-push` は id重複、card-data重複、GA欠落、触ったEN slugの内容消失、EN keyword chip のリンク消失、JA/EN の §REL・本文節の非対称などを検査する。FAILなら push しない。緊急回避は `git push --no-verify`。
  ...
  - 分類ページ本文消失: `check_classification_loss()` が触った `archive.html` / `eras/*` / `movements/*` と **`en/archive.html` / `en/eras/*` / `en/movements/*`**（いずれもHTML正本）を baseline 比較し、`<main>`領域消失・`<h1>`消失・`pc-card`数の減少・**解説文の半減**をHARD、section/リンク/data-nosnippet の減少・**解説文の15%以上かつ200文字以上の減少**をWARN。解説文の文字数はカード・AI開示を除いて数える（カード本文を含めると解説文の消失が埋もれる）。国別はJSON正本（再生成で戻る）のため対象外。
